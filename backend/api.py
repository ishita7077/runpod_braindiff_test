...
def _pipeline_label(modality: str) -> str:
    ...
    FFmpeg-extracted frames + audio). Mirrors the worker's `_pipeline_label`.
    """
    if modality == "text":
...
def _build_diff_result(
    *,
    ...
    # results recall card both have the actual content of the stimulus rather
    # than a "Text A · 0 chars" placeholder.
    if modality == "text":
    ...
    if modality == "text":
        # Back-compat: text mode still exposes text_a/text_b at the top level
        # because the recall card and tests both read them.
...
def _run_diff_job(job_id: str, request_id: str, payload: DiffRequest) -> None:
    ...
    try:
        ...
        # which run.html's Deep Scope reads. The model_service call itself is
        # monolithic; we wrap it so the user sees real stage transitions,
        # not invented ones.
        ...
        class _StoreEmitter:
            ...
            def emit(self, status: str, message: str) -> None:
                # Map model-side status names to per-side ones the frontend
                # already understands.
                if status == "transcribing":
                    job_store.update_status(
                        job_id, f"transcribing_version_{self.side}", message
                    )
                elif status == "predicting":
                    job_store.update_status(
                        job_id, f"predicting_version_{self.side}", message
                    )
                elif status == "synthesizing_speech":
                    # Already emitted before the prediction call kicked off.
        ...
        # so the insight engine and results recall card both have access to
        # the actual content of the stimulus, not just zeros.
