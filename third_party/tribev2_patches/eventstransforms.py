...
class ExtractWordsFromAudio(EventsTransform):
    ...
    def _get_transcript_from_audio(wav_filename: Path, language: str) -> pd.DataFrame:
        ...
        # BrainDiff (and operators) set these before loading TRIBEv2; upstream ignored them.
        device = os.environ.get("TRIBEV2_WHISPERX_DEVICE", "").strip().lower()
        if not device:
            device = "cuda" if torch.cuda.is_available() else "cpu"
        model_name = os.environ.get("TRIBEV2_WHISPERX_MODEL", "large-v3").strip() or "large-v3"
        batch_size = os.environ.get("TRIBEV2_WHISPERX_BATCH_SIZE", "16").strip() or "16"
        align_default = "WAV2VEC2_ASR_LARGE_LV60K_960H" if language == "english" else ""
        align_model = os.environ.get("TRIBEV2_WHISPERX_ALIGN_MODEL", align_default).strip()

        # CTranslate2 (faster-whisper): float16 is for CUDA. CPU raises
        # "Requested float16 compute type, but the target device or backend do not support efficient float16 computation."
        compute_type = os.environ.get("TRIBEV2_WHISPERX_COMPUTE_TYPE", "").strip().lower()
        if not compute_type:
