...
- The status endpoint (api/diff/status/[jobId].js) reads the list with LRANGE
  and returns it to the frontend; run.html's progress UI consumes it.
...
class RedisProgressEmitter:
    ...
    def emit(self, status: str, message: str) -> None:
        ...
        # it to push and refresh the TTL atomically. RPUSH preserves arrival
        # order; EXPIRE keeps the list from outliving the job.
